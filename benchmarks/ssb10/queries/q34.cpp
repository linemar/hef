 ...
 * @LastEditTime: 2022-01-23 18:08:56
 * @Description:   
 * @FilePath: /sunzw/workspace/ecnu/ecnu/hef/benchmarks/ssb10/queries/q34.cpp
 */
...
const int customer_hash_tabel_size = 64 * 1024;
const int supplier_hash_tabel_size = 4 * 1024;
const int date_hash_tabel_size = 1024;

double q34_scalar(DataBase *db)
{
   ...
   std::string *d_yearmonth = (std::string *)t_date.get_column("d_yearmonth").get_data();
   uint64_t *p_partkey = (uint64_t *)t_part.get_column("p_partkey").get_data();
   std::string *c_city = (std::string *)t_customer.get_column("c_city").get_data();
   uint64_t *s_suppkey = (uint64_t *)t_supplier.get_column("s_suppkey").get_data();
   std::string *s_city = (std::string *)t_supplier.get_column("s_city").get_data();
   ...
   uint64_t c_size = t_customer.get_column("c_city").get_size();
   uint64_t s_size = t_supplier.get_column("s_city").get_size();
...
double q34_hi(DataBase *db, uint64_t batch_size)
{
...
double q34_hi_block(DataBase *db, uint64_t batch_size)
{
...
int main()
{
   ...
   runtime = q34_hi(db, 36);
   std::cout << std::setiosflags(std::ios::fixed) << std::setprecision(6) << runtime << std::endl;
   // sleep(30);

   // runtime = q34_hi_block(db, 36);
   // std::cout << std::setiosflags(std::ios::fixed) << std::setprecision(6) << runtime << std::endl;

 ...
 * @LastEditTime: 2022-01-21 13:23:39
 * @Description:   
 * @FilePath: /sunzw/workspace/ecnu/ecnu/hef/benchmarks/ssb10/queries/q22.cpp
 */
...
double q22_hi(DataBase *db, uint64_t batch_size)
{
...
double q22_hi_block(DataBase *db, uint64_t batch_size)
{
...
int main()
{
    ...
    runtime = q22_hi(db, 36);
    std::cout << std::setiosflags(std::ios::fixed) << std::setprecision(6) << runtime << std::endl;

    // sleep(30);

    // runtime = q22_hi_block(db, 36);
    // std::cout << std::setiosflags(std::ios::fixed) << std::setprecision(6) << runtime << std::endl;

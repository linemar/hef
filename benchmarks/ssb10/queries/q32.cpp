 ...
 * @LastEditTime: 2022-01-21 13:36:16
 * @Description:   
 * @FilePath: /sunzw/workspace/ecnu/ecnu/hef/benchmarks/ssb10/queries/q32.cpp
 */

/*
Q2.1
select sum(lo_revenue), d_year, c_nation
    from lineorder, date, part, supplier
    where lo_orderdate = d_datekey
        and lo_partkey = p_partkey
        and lo_suppkey = s_suppkey
        and c_nation = 'MFGR#12'
...
double q32_scalar(DataBase *db)
{
   ...
   std::string *c_nation = (std::string *)t_customer.get_column("c_nation").get_data();
   uint64_t *s_suppkey = (uint64_t *)t_supplier.get_column("s_suppkey").get_data();
   std::string *s_nation = (std::string *)t_supplier.get_column("s_nation").get_data();
   ...
   uint64_t c_size = t_customer.get_column("c_nation").get_size();
   uint64_t s_size = t_supplier.get_column("s_nation").get_size();
   ...
   for (uint64_t i = 0; i < c_size; i++)
   {
      if (0 == std::string(c_nation[i]).compare("UNITED STATES"))
      {
...
double q32_hi(DataBase *db, uint64_t batch_size)
{
   ...
   for (uint64_t i = 0; i < lo_size - rest; i = i + block_size)
   {
      ...
      // hashjoin_i64_l1(customer_hash_table, customer_hash_tabel_size, lo_rid + i, lo_custkey  + i, block_size, batch_size, lo_result, lo_result_size);
...
double q32_hi_block(DataBase *db, uint64_t batch_size)
{
...
int main()
{
   ...
   runtime = q32_hi(db, 36);
   std::cout << std::setiosflags(std::ios::fixed) << std::setprecision(6) << runtime << std::endl;
   // sleep(30);

   // runtime = q32_hi_block(db, 36);
   // std::cout << std::setiosflags(std::ios::fixed) << std::setprecision(6) << runtime << std::endl;

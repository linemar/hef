 ...
 * @LastEditTime: 2022-01-21 13:23:48
 * @Description:   
 * @FilePath: /sunzw/workspace/ecnu/ecnu/hef/benchmarks/ssb10/queries/q23.cpp
 */
...
double q23_scalar(DataBase *db)
{
    ...
    std::string *p_brand = (std::string *)t_part.get_column("p_brand").get_data();
...
double q23_hi(DataBase *db, uint64_t batch_size)
{
...
double q23_hi_block(DataBase *db, uint64_t batch_size)
{
...
int main()
{
    ...
    runtime = q23_hi(db, 36);
    std::cout << std::setiosflags(std::ios::fixed) << std::setprecision(6) << runtime << std::endl;
    // sleep(30);

    // runtime = q23_hi_block(db, 36);
    // std::cout << std::setiosflags(std::ios::fixed) << std::setprecision(6) << runtime << std::endl;

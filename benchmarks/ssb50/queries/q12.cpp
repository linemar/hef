...
NOVECTORIZE std::unique_ptr<runtime::Query> q12_hyper(Database& 
                                                      size_t nrThreads) {
   // --- aggregates
...
std::unique_ptr<runtime::Query> q12_vectorwise(Database&  size_t nrThreads,
                                               size_t vectorSize) {
   using namespace vectorwise;
   ...
   n = 0;
   workers.run([&]() {
      Q12Builder b( shared, vectorSize);

 ...
 * @LastEditTime: 2022-01-21 21:01:19
 * @Description:   
 * @FilePath: /sunzw/workspace/ecnu/ecnu/hef/benchmarks/ssb50/queries/q23.cpp
 */
...
const int part_hash_tabel_size = 32 * 1024;
...
double q23_scalar()
{
    ...
    {
        if (0 == std::string(p_brand[i]).compare("MFGR#2339"))
        {
    ...
    {
        if (0 == std::string(s_region[i]).compare("EUROPE"))
        {
...
double q23_hi( uint64_t batch_size)
{
    uint64_t *lo_rid = new uint64_t[lo_size];
...
double q23_hi_block( uint64_t batch_size)
{
    uint64_t *lo_rid = new uint64_t[lo_size];
...
int main()
{
    ...
    size = load_data_str(str_data1, std::string("/home/sunzw/data/ssb/scale_100/part_brand.txt"));

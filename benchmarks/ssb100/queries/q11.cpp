 ...
 * @Description:   
 * @FilePath: /sunzw/workspace/ecnu/ecnu/hef/benchmarks/ssb/queries/q11.cpp
 */
...
double q11( uint64_t batch_size)
{
    const int hash_tabel_size = 512;

    //get table data
    Table t_date = ->get_table(std::string("date"));
    Table t_lineorder = ->get_table(std::string("lineorder"));
    ...
    uint64_t *lo_quantity = (uint64_t *)t_lineorder.get_column("lo_quantity").get_data();
    uint64_t *lo_discount = (uint64_t *)t_lineorder.get_column("lo_discount").get_data();
    ...
    std::cout << "finished, hash tabel size is " << (unsigned int)(hash_tabel_size) << ", use time " << dr_ms << " ms" << std::endl;

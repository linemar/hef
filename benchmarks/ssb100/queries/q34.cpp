 ...
 * @LastEditTime: 2022-01-23 16:56:45
 * @Description:   
 * @FilePath: /sunzw/workspace/ecnu/ecnu/hef/benchmarks/ssb50/queries/q34.cpp
 */

/*
Q2.1
select sum(lo_revenue), d_yearmonth, c_city
    ...
    where lo_orderdate = d_datekey
        ...
        and c_city = 'MFGR#12'
        and s_city = 'AMERICA'
        group by d_yearmonth, c_city
        order by d_yearmonth, c_city;
*/
...
const int customer_hash_tabel_size = 256 * 1024;
const int supplier_hash_tabel_size = 16 * 1024;
const int date_hash_tabel_size = 1024;
...
double q34_scalar()
{
   ...
   {
      if (0 == d_yearmonth[i].compare("Dec1997"))
      {
...
double q34_hi(uint64_t batch_size)
{
   uint64_t *lo_rid = new uint64_t[lo_size];
...
double q34_hi_block(uint64_t batch_size)
{
   uint64_t *lo_rid = new uint64_t[lo_size];
...
int main()
{
   ...
   d_size = load_data_int(int_data1, std::string("/home/sunzw/data/ssb/scale_10_spilt/date_key.txt"));
   d_datekey = int_data1.data();
   std::cout << "d_size = " << d_size << std::endl;

   std::vector<uint64_t> int_data2;
   c_size = load_data_int(int_data2, std::string("/home/sunzw/data/ssb/scale_10_spilt/customer_key.txt"));
   c_custkey = int_data2.data();
   std::cout << "p_size = " << c_size << std::endl;

   std::vector<std::string> str_data1;
   size = load_data_str(str_data1, std::string("/home/sunzw/data/ssb/scale_10_spilt/customer_city.txt"));
   c_city = str_data1.data();

   std::vector<uint64_t> int_data3;
   s_size = load_data_int(int_data3, std::string("/home/sunzw/data/ssb/scale_10_spilt/supplier_key.txt"));
   s_suppkey = int_data3.data();
   std::cout << "s_size = " << s_size << std::endl;

   std::vector<std::string> str_data2;
   size = load_data_str(str_data2, std::string("/home/sunzw/data/ssb/scale_10_spilt/supplier_city.txt"));
   s_city = str_data2.data();

   std::vector<uint64_t> int_data4;
   lo_size = load_data_int(int_data4, std::string("/home/sunzw/data/ssb/scale_10_spilt/lineorder_date.txt"));
   lo_orderdate = int_data4.data();
   std::cout << "lo_size = " << lo_size << std::endl;

   std::vector<uint64_t> int_data5;
   size = load_data_int(int_data5, std::string("/home/sunzw/data/ssb/scale_10_spilt/lineorder_customer.txt"));
   lo_custkey = int_data5.data();

   std::vector<uint64_t> int_data6;
   size = load_data_int(int_data6, std::string("/home/sunzw/data/ssb/scale_10_spilt/lineorder_supplier.txt"));
   lo_suppkey = int_data6.data();

   std::vector<std::string> str_data7;
   size = load_data_str(str_data7, std::string("/home/sunzw/data/ssb/scale_10_spilt/date_year.txt"));
   d_yearmonth = str_data7.data();

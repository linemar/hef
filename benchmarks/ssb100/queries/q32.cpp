 ...
 * @LastEditTime: 2022-01-23 17:53:16
 * @Description:   
 * @FilePath: /sunzw/workspace/ecnu/ecnu/hef/benchmarks/ssb50/queries/q32.cpp
 */
...
double q32_hi(uint64_t batch_size)
{
   uint64_t *lo_rid = new uint64_t[lo_size];
   for (uint64_t i = 0; i < lo_size; i++)
   {
...
double q32_hi_block(uint64_t batch_size)
{
   uint64_t *lo_rid = new uint64_t[lo_size];
...
int main()
{
   ...
   c_size = load_data_int(int_data2, std::string("/home/sunzw/data/ssb/scale_50_spilt/customer_key.txt"));
   c_custkey = int_data2.data();
   std::cout << "c_size = " << c_size << std::endl;

   std::vector<std::string> str_data1;
   size = load_data_str(str_data1, std::string("/home/sunzw/data/ssb/scale_50_spilt/customer_nation.txt"));
   c_nation = str_data1.data();
   ...
   size = load_data_int(int_data5, std::string("/home/sunzw/data/ssb/scale_50_spilt/lineorder_customer.txt"));

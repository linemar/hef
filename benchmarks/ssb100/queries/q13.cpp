...
NOVECTORIZE std::unique_ptr<runtime::Query> q13_hyper(Database& 
                                                      size_t nrThreads) {
   // --- aggregates
...
std::unique_ptr<runtime::Query> q13_vectorwise(Database&  size_t nrThreads,
                                               size_t vectorSize) {
   using namespace vectorwise;
   ...
   n = 0;
   workers.run([&]() {
      Q13Builder b( shared, vectorSize);
